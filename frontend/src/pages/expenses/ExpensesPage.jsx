...
function ExpensesPage() {
    ...
    api
      ...
      .catch((requestError) => {
        if (requestError.code !== "ERR_CANCELED") {
          setError(
            extractApiError(requestError, "Unable to load expense filters."),
          );
        }
      });
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((requestError) => {
        if (requestError.code !== "ERR_CANCELED") {
          setError(extractApiError(requestError, "Unable to load expenses."));
        }
      })
      .finally(() => {
  ...
  const handleApprove = async (expense) => {
    ...
    try {
      ...
      setError("");
      const response = await api.patch(`/expenses/${expense.id}/approve`);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Track operating costs, approvals, due dates, and payments.
            </p>
          </div>
        </div>
              ...
              {loading ? (
                ...
                expenses.map((expense) => (
                  <tr key={expense.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="px-5 py-4 text-sm text-slate-600">
                      <p>{expense.location?.name ?? "Company-wide"}</p>
        ...
        {meta && (
          <div className="flex items-center justify-between border-t border-slate-200 px-5 py-4 text-sm">
            <span className="text-slate-500">
              {meta.total ?? expenses.length} expenses
            </span>
            <div className="flex items-center gap-3">
              <button
                type="button"
                disabled={!meta.prev_page_url}
                onClick={() => changePage(Math.max(request.page - 1, 1))}
