...
function ExpenseDetails() {
  ...
  const handleApprove = async () => {
    if (!window.confirm(`Approve expense ${expense.expense_number}?`)) return;
    ...
    try {
      ...
      setMessage(response.data.message ?? "Expense approved successfully.");
    } catch (requestError) {
      setError(extractApiError(requestError, "Unable to approve expense."));
    } finally {
      setActing(false);
    }
  };
  ...
  const handleCancel = async () => {
    ...
    if (!reason.trim()) {
      setError("The cancellation reason is required.");
      return;
    }
    ...
    try {
      ...
      setExpense(response.data.data);
      setMessage(response.data.message ?? "Expense cancelled successfully.");
    } catch (requestError) {
      setError(extractApiError(requestError, "Unable to cancel expense."));
    } finally {
      setActing(false);
    }
  };
  ...
  const documentUrl = getDocumentUrl(expense.document_path);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <InfoCard icon={ReceiptText} label="Category">
          <p className="font-bold text-slate-900">
            {expense.category?.name ?? "-"}
          </p>
          <p className="mt-1 text-xs text-slate-400">
            {expense.bill_reference || "No bill reference"}
          </p>
        </InfoCard>
        ...
        <InfoCard icon={CalendarDays} label="Dates">
          <p className="font-bold text-slate-900">
            Issued {formatDate(expense.issue_date)}
          </p>
          <p className="mt-1 text-xs text-slate-400">
            Due {formatDate(expense.due_date)}
          </p>
        </InfoCard>
        <InfoCard icon={UserRound} label="Workflow">
          <p className="font-bold text-slate-900">
          ...
          <div className="grid gap-5 p-6 sm:grid-cols-2">
            ...
            <div>
              ...
              {documentUrl ? (
                ...
                >
                  <Download size={16} /> Open supporting document
                </a>
              ) : (
          ...
          <div className="mt-5 space-y-3 text-sm">
            <div className="flex justify-between gap-3 text-slate-600">
              ...
              <strong className="text-slate-900">
                {formatMoney(expense.amount_ht)} MAD
              </strong>
            </div>
            <div className="flex justify-between gap-3 text-slate-600">
              <span>TVA ({getTaxPercentage(expense)}%)</span>
              <strong className="text-slate-900">
                {formatMoney(expense.tax_amount)} MAD
              </strong>
            </div>
            ...
            <div className="flex justify-between gap-3 rounded-xl bg-slate-900 px-4 py-3 text-white">
              ...
              <strong>{formatMoney(expense.remaining_amount)} MAD</strong>
            </div>
          </div>
        </section>
      </div>
